...
`next_design`, `human_rule`, `human_rule_issue` and `human_close` — find the
subject, read it, audit its source, rule on it, close it. The human has
**three** inboxes, so it has three "which is next" tools: `next_ready` for PRs,
`next_close_candidate` for close-candidate flags (#173), and `next_design` for
open design questions (#220), oldest question first. `human_close` is a tool
rather than something the caller composes for the reason above: the alternative
is a transition half in a tool and half in a prompt, and that half was wrong on
all 74 closed-and-still-flagged subjects. The subcommands above are for the
human at a terminal; the profile is for **an agent acting on the human's
