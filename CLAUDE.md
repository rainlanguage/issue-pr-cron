...
| `human`            | `next_ready`, `pr_context`, `pr_checkout`, `clone_release`, `next_close_candidate`, `close_candidate_context`, `next_design`, `human_rule`, `human_rule_issue`, `human_close` — find the subject, read it, audit its source, rule on it. THREE inboxes means three "which is next" reads: `next_ready` for PRs, `next_close_candidate` for flags, `next_design` for design questions |
