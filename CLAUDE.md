...
| `human`            | `next_ready`, `pr_context`, `pr_checkout`, `clone_release`, `next_close_candidate`, `close_candidate_context`, `next_leak`, `human_rule`, `human_rule_issue`, `human_close` — find the subject, read it, audit its source, rule on it. Each inbox has its own "which is next" read: `next_ready` for PRs, `next_close_candidate` for flags, `next_leak` for the FSM-conformance leaks — the inbox that should be EMPTY |
