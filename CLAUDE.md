...
| `already-fixed <owner/repo#n>...`                                  | has a MERGED PR referencing this issue landed SINCE it was filed? The question `uncovered-issues` cannot answer — that split is computed from OPEN PRs only, so an issue fixed on `main` with no open PR pointing at it reads as uncovered and gets worked. Takes ISSUE refs (work this or not) and PR refs (each issue the PR closes, so supersession is detected). Exit 4 = merged-since, 1 = unreadable, 0 = clear; 4 is a reason to READ that PR, never a finding |
...
| `work-tokens <metrics/runs.jsonl> [--json]`                        | TOKENS TO LAND WORK: per-task spend joined to the work items `open_pr` recorded, bucketed landed / delivered-awaiting-human / churn. Only churn is waste; a task with no typed item is churn and nothing is inferred from a dispatch label                                                                                                                                                                                                                            |
...
| `producer`         | `clone_create`, `clone_release`, `clone_list`, `clone_gc`, `open_pr`, `repair_qa_block`, `weaken_closes` — the clone lifecycle, the OUTPUT edge, plus the two body repairs                                                                                                                                                    |
