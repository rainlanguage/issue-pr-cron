...
One skill. The [issue→PR pipeline](https://github.com/rainlanguage/issue-pr-cron)'s
vetting prompt states the **gates**; this states their **calibration** — the
...
server, takes no argument and writes no GitHub state; it is calibration for gates
the prompt already mandates, and where the two appear to disagree the prompt is
what `pr-review-report` enforces. It is also **not an audit lens** — a verdict
still requires a `pr_checkout` tree at the PR's head and an `audit` invocation
naming that PR at a declared `pr:<number>` scope, and only a skill whose id ends
in `audit` is ever credited as one.
