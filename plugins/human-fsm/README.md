...
| `/reject <owner/repo#n> "…"`                 | `human-rule … reject --rework` / `human-rule-issue` — the send-back: `ai:reject` (PR) plus the trusted `Rework note` work order, one call, pinned to the head sha or the issue                                           |
| `/design <owner/repo#n> "…"`                 | `human-rule … design --rework\|--park` / `human-rule-issue` — `human:design`, delegated as a work order or explicitly parked, never parked by default                                                                    |
