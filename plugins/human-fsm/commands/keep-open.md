...
`keep-open` is **issue-only**: it answers a producer close-candidate flag with "no",
and a PR has no such flag to answer — `human:keep-open` on a PR is a label the lane
classifier buckets nowhere, so the PR would silently leave the human-decisions lane
entirely. The tool refuses it there and says so.

This is the one ruling that **clears** an `ai:*` label: `keep-open` contradicts
`ai:close-candidate` outright, so the flag goes with it. The comment pins to the live
flag's timestamp, exactly as the vetter's own verdict on that flag does, so a re-flag
would stale both records together.

It is the **sacred** answer — the issue is never re-flagged. If you only mean "not on
this evidence, the producer may try again", that is `/close-candidate <ref> reject`.
